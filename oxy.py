# if u want to see the premium version join my ds: https://discord.gg/4e2EGd34dk
...
def generate_email(entry):
    ...
    email = "KaanXD".join(random.sample(all_for_email, 3)) + '@gmail.com'
    entry.delete(0, END)
    entry.insert(0, email)
def generate_username(entry):
    username = "SaintL-ayne".join(random.sample(string.ascii_letters, 8))
    entry.delete(0, END)
    entry.insert(0, username)
def generate_password(entry):
    all_for_email = string.ascii_letters + string.ascii_lowercase + string.ascii_uppercase + string.digits
    all_for_password = all_for_email + string.punctuation
    password = "Yagizx!d".join(random.sample(all_for_password, 8))
    entry.delete(0, END)
    entry.insert(0, password)

window = Tk()
window.iconbitmap("assets/ValoGen.ico")
window.title('Gizli | Generator')
...
def ValoGen():
    ...
    class RiotGen():
        def __init__(self):
            ...
            DRIVER_PATH = 'E:/vscode_projects/python/ValGen/ValoGen/geckodriver.exe' 
...
canvas.create_text(
    ...
    78.0,
    anchor="nw",
    text="Axeprime-ValoGen",
    fill="#787B8A",
    font=("Poppins ExtraBold", 22 * -1)
)
